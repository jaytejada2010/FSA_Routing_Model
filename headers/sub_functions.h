...
void printDistanceMatrix(Matrix *distance, int num_of_nodes);
...
Matrix *initializeDistanceMatrix()
{
    ...
    // printDistanceMatrix(distance, num_of_nodes);
    // cout << "\n\n" << "Distance Matrix: " << "\n";
...
void displayFlamingoPopulation(vector<Flamingo> fl, string filename, int iterations)
{
    ofstream file;
...
void printDistanceMatrix(Matrix *distance, int num_of_nodes)
{
    ...
    for (int x = 0; x < num_of_nodes; x++)
    {
        for (int y = 0; y < num_of_nodes; y++)
        {
            file << static_cast<int>((*distance)[x][y]) << "\t";
        }
        file << "\n\n";
    }
...
void printTime(vector<double> maxDiff, string filename, double time_spent)
{
    ofstream file;
    file.open(filename, ios::app);

    int ctr = 0;
    for (vector<double>::iterator m = maxDiff.begin(); m != maxDiff.end(); m++, ctr++)
    {
        file << endl
             << "Iteration " << ctr << " Max Delta: " << *m;
    }
